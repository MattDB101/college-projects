...
def split():
    ...
    print(f"Positive reviews in the training set: {trainingLabels.value_counts()['positive']}") # Task 1.2
    ...
    return trainingData, trainingLabels, testingData, testingLabels # Task 1.1



def extract(trainingData, minLen, minCount):
    trainingData = trainingData.replace("[^a-zA-Z0-9 ]", "", regex=True) # Task 2.1
    trainingData = trainingData.str.lower() # Task 2.2
    trainingData = trainingData.str.split(" ") # Task 2.3
    ...
    occList = pd.value_counts(np.hstack(trainingData)).rename_axis("Word").reset_index(name="Occurrences") # Task 2.4 horizontaly stack then count the unique values, create a new df with labels Word & Occurrences
    occList = occList[occList.Occurrences >= minCount] # Task 2.5 drop rows where the count is less than specified
    occList = occList[occList["Word"].str.len() >= minLen] # Task 2.5 drop rows where the word length is less than specified
...
def countReviews(reviews, extractedWords):
    ...
    #print(wordDict)
    return wordDict # Task 3



def calculateLikelihoods(positiveReviewWords, positiveReviewsCount, negativeReviewsWords, negativeReviewsCount):
    priorPos = positiveReviewsCount / (positiveReviewsCount + negativeReviewsCount) # Task 4.2 calculate priors 
    ...
    for word in positiveReviewWords: # how many SENTIMENT reviews the word appears in, divided by the total count of SENTIMENT reviews.
        wordsPosLikelihood[word] = (positiveReviewWords[word] + 1) / (positiveReviewsCount + (1 * 2)) # Task 4.1
...
def predict(review, wordsPosLikelihood, wordsNegLikelihood, priorPos, priorNeg):
    ...
    if logLikelihoodPos - logLikelihoodNeg > math.log(priorNeg) - math.log(priorPos): # Task 5
        return "positive"
    else:
        return "negative"
...
def evaluate(trainingData, trainingLabels, testingData, testingLabels):
    ...
    for k in range(1,11):
        ...
        for train_index, test_index in kf.split(trainingData, trainingLabels): 
            ...
            for review in trainingData.iloc[test_index]:
                prediction.append(predict(review, wordsPosLikelihood, wordsNegLikelihood, priorPos, priorNeg)) # Task 6.1
        ...
        meanAccuracy = accuracy / numSplits  # Task 6.3
        meanAccuracyList.append(meanAccuracy)  
        print(f"Mean Accuracy: {meanAccuracy}")
        
    print(meanAccuracyList) # Task 6.4
    print(f"Best: {max(meanAccuracyList)} MinLen: {meanAccuracyList.index(max(meanAccuracyList))+1}" ) # Task 6.5
    ...
    confusion = metrics.confusion_matrix(testingLabels, prediction) # Task 6.6
    
    print(confusion)
    tp = (confusion[0,0] / np.sum(confusion)) * 100 # Task 6.7
    fp = (confusion[0,1] / np.sum(confusion)) * 100 # Task 6.8
    fn = (confusion[1,0] / np.sum(confusion)) * 100 # Task 6.9
    tn = (confusion[1,1] / np.sum(confusion)) * 100 # Task 6.10
    ...
    accuracy = metrics.accuracy_score(testingLabels, prediction) # Task 6.11
