...
def naive_bayes():
    ...
    test_index = np.arange(int(0.8*len(target)), len(target)) #splitting the dataset

    total = len(target[train_index])
    survivors = sum(target[train_index]==1)
    casulties = sum(target[train_index]==0) # counting each category
    print("Total number of passengers:",total)    
    print("Survivors:",survivors)
    print("Casulties:",casulties)

    prior_survivors = survivors / total # calculating priors
    prior_casulties = casulties / total
    print("Prior survivors:", prior_survivors)
    print("Prior casulties:", prior_casulties)

    male_survivors = sum((data.iloc[train_index]["Sex"]=="male") & (target[train_index]==1)) # calculate probability of survival given class male
