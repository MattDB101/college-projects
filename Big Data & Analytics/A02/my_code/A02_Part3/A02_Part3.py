...
def my_main(spark, my_dataset_dir, bike_id):
    ...
    inputDF = inputDF.where(col('bike_id') == bike_id)
    inputDF = inputDF.withColumn('prev_station', lag('stop_station_name', 1).over(Window.orderBy('start_time')))
    inputDF = inputDF.withColumn('prev_time', lag('stop_time', 1).over(Window.orderBy('start_time')))
    inputDF = inputDF.where(col('start_station_name') != col('prev_station'))

    solutionDF = inputDF.select(col('prev_time').alias('start_time'),
                col('prev_station').alias('start_station_name'),
    ...
    #Operation A1: 'collect' to get all resultspyspark
