...
def my_main(spark, my_dataset_dir):
    ...
    start_stations = inputDF.groupBy(col('start_station_name').alias('station'))
    stop_stations = inputDF.groupBy(col('stop_station_name').alias('station'))

    start_stations = start_stations.agg(count('start_station_name').alias('num_departure_trips'))
    stop_stations = stop_stations.agg(count('stop_station_name').alias('num_arrival_trips'))

    solutionDF = start_stations.join(stop_stations, 'station', 'outer').orderBy(col('station')).fillna(0)
