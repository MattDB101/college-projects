...
def my_main(spark, my_dataset_dir, top_n_bikes):
    ...
    inputDF = inputDF.groupBy("bike_id")
    inputDF = inputDF.agg(sum("trip_duration").alias("totalTime"), count("trip_id").alias("numTrips"))
    inputDF = inputDF.sort("totalTime", ascending=False)
    solutionDF = inputDF.limit(top_n_bikes)
