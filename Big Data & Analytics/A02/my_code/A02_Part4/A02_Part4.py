...
# ------------------------------------------
import pyspark
from pyspark.sql.functions import col, count, current_timestamp, window
...
def my_model(spark,
    ...
    inputSDF = inputSDF.withWatermark("my_time", "0 seconds").groupBy(window("my_time", my_frequency, my_frequency), col('start_station_name'))
    inputSDF = inputSDF.agg(count(col('start_station_name')).alias('num_departures'))
    inputSDF = inputSDF.filter(col('num_departures') >= min_trips)
    inputSDF = inputSDF \
        .withColumn("window_start", col("window").start.cast("string")) \
        .withColumn("window_end", col("window").end.cast("string"))

    solutionSDF = inputSDF.drop("window").select('start_station_name', 'num_departures')
...
if __name__ == '__main__':
    ...
    # 1.1. We specify the minimum number of trips
    min_trips = 8
    ...
    source_dir = "../../my_datasets/my_streaming_dataset/"
    monitoring_dir = "../../my_datasets/my_monitoring/"
    checkpoint_dir = "../../my_datasets/my_checkpoint/"

    if local_False_databricks_True == True:
        source_dir = "/FileStore/tables/Assignments/NYC/my_streaming_dataset/"
        monitoring_dir = "/FileStore/tables/Assignments/NYC/my_monitoring/"
        checkpoint_dir = "/FileStore/tables/Assignments/NYC/my_checkpoint/"
